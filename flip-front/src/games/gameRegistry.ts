 ...
 */
    register(metadata: GameMetadata, options: Omit<GameRegistryEntry, 'metadata'> = { isEnabled: true }) {
...
export const navigateToGame = (navigation: any, gameId: string, players: any[]) => {
    ...
    if (players.length < game.minPlayers || players.length > game.maxPlayers) {
        throw new Error(`Nombre de joueurs invalide pour ce jeu (${game.minPlayers}-${game.maxPlayers})`);
    }
