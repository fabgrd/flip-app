...
import type { CameleonAssignedPlayer, CameleonGameState, CameleonRoleDistribution, StartCameleonOptions } from '../types';
...
export function useCameleon(initialPlayers: Player[]) {
    const [gameState, setGameState] = useState<CameleonGameState>({
        players: initialPlayers.map(p => ({ ...p, role: 'civilian', secretWord: null, isEliminated: false } as CameleonAssignedPlayer)),
        wordPair: null,
        round: 0,
        started: false,
    });
    ...
    const startGame = useCallback((options?: StartCameleonOptions) => {
        const override = options?.overrideDistribution ?? {};
        const maxImpostors = Math.floor(playerCount / 2);
        let undercovers = Math.max(0, Math.min(playerCount, override.undercovers ?? defaultDistribution.undercovers));
        let mrWhites = Math.max(0, Math.min(playerCount - undercovers, override.mrWhites ?? defaultDistribution.mrWhites));
        ...
        for (let i = 0; i < undercovers; i++) {
            ...
            if (!base) break;
            assigned.push({ ...base, role: 'cameleon', secretWord: wordPair.cameleonWord, isEliminated: false });
        }
        ...
        for (let i = 0; i < civilians; i++) {
            const base = shuffledPlayers.shift();
            if (!base) break;
            assigned.push({ ...base, role: 'civilian', secretWord: wordPair.civilianWord, isEliminated: false });
        }
    ...
    const currentRevealPlayer = useMemo(() => gameState.players[revealIndex] ?? null, [gameState.players, revealIndex]);
    ...
    const confirmElimination = useCallback(() => {
        ...
        setGameState(prev => {
            const updatedPlayers = prev.players.map(p => p.id === selectedForElimination ? { ...p, isEliminated: true } : p);
    ...
    const submitMrWhiteGuess = useCallback((guessRaw: string) => {
        ...
        setGameState(prev => {
            ...
            const updatedPlayers = prev.players.map(p => {
                ...
                return { ...p, scoreBonus: current + bonus, mrWhiteGuess: guess, mrWhiteGuessCorrect: isCorrect } as CameleonAssignedPlayer;
            });
